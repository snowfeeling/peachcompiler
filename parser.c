...
void parse_single_token_to_node()
{
    ...
    case TOKEN_TYPE_STRING:
        ...
        break;
    default:
        compiler_error(current_process, "This is not a single that can be converted to a node");
        break;
    }
}
...
void parse_exp_normal(struct history *history)
{
    ...
    if (!node_left)
    {
        compiler_error(current_process, "Expected a left node");
        return;
    }
    // pop off the operator token
    struct token *tmpToken = token_next(); // wangss changed ： token_next();
    // pop off the left node
    struct node *tmpNode = node_pop(); // wangss changed ： node_pop();
...
void parser_datatype_init_type_and_size(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out, int pointer_depth, int expected_type)
{
    ...
    */
    case DATA_TYPE_EXPECT_STRUCT:
    case DATA_TYPE_EXPECT_UNION:
        compiler_error(current_process, "Struct and union are currently unsupprted.\n");
        break;
    default:
